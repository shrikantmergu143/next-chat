...
const validate = (value?:any, rules?:any, label?:any, path?:any) => {
  if (rules.required) {
    if (!value) {
    ...
    const FormControl = () => {
      ...
      if (props?.type === "checkbox" || props?.type === "switch" || props?.type === "switch-1") {
        if(props?.type === "switch-1"){
          return(
            <div className="ios-toggle-mega-label-wrap">
              <span className="ios-toggle ios-toggle-reverse">
                <input type="checkbox" id={`include-forks-${ids}`}
                  ...
                  className={`${className} ${error ? "is-invalid" : ""}`}
                />
                <label htmlFor={`include-forks-${ids}`}></label>
              </span>
              {props?.label && <label htmlFor={`include-forks-${ids}`} className="form-check-label">{props?.label}</label>}
            </div>
          )
        }
        return (
