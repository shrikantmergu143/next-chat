...
const Editor = ({ field, html, classes, onChange, placeholder, onSend }:any) => {
  ...
  const onSendMessage = () =>{
    const initialHtml = html;
    const el = editorRef.current;
    const newHtml: any = el ? el.innerHTML : (event?.target as HTMLInputElement)?.value;
        ...
        <div className="position-relative h-100 w-100">
          <div className={`text-editor ${classes ? classes : ""}`}>
            ...
            <div
              ...
              onBlur={(event) => handleSave(event, false)}
              onPaste={handlePaste}
